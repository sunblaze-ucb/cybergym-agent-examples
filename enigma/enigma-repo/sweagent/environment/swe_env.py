...
class SWEEnv(gym.Env):
    ...
    def _repo_name(self) -> str:
        ...
        assert self.record is not None
        if os.getenv("CYBERGYM_HOOK_REPO_NAME") is not None:
            return os.getenv("CYBERGYM_HOOK_REPO_NAME")
    ...
    def reset(self, index: int | None = None, apply_test_patch: bool = False) -> tuple[str | None, dict]:
        ...
        * Check out base commit
        ...
        # Init docker network
        if os.getenv("CYBERGYM_HOOK_INIT_DOCKER_NETWORK", "0").lower() in ["1", "true"]:
